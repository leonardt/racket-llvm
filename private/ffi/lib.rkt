...
(define llvm-version-string
  (let-values (((process out in err) (subprocess #f #f #f "/usr/bin/env" (llvm-config-path) "--version")))
    (begin0
...
(define llvm-lib-path
  (let-values (((process out in err) (subprocess #f #f #f "/usr/bin/env" (llvm-config-path) "--libdir")))
    (begin0
        (string-trim-both (port->string out))
      (close-output-port in)
      (close-input-port err)
